...
namespace ImGui
{
	...
	IMGUI_API void PushFontSize(ImFont* font, float size);
	IMGUI_API void PopFontSize(int size = 1);

	IMGUI_API bool CenterButton(const char* label, const ImVec2& size_arg = {0.f, 0.f});

	IMGUI_API bool Spinner(const char* label, float radius, int thickness = 1, const ImU32& color = 0);
